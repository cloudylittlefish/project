<html lang="zh-CN" class=" js flexbox canvas canvastext webgl no-touch geolocation postmessage websqldatabase indexeddb hashchange history draganddrop websockets rgba hsla multiplebgs backgroundsize borderimage borderradius boxshadow textshadow opacity cssanimations csscolumns cssgradients cssreflections csstransforms csstransforms3d csstransitions fontface generatedcontent video audio localstorage sessionstorage webworkers applicationcache svg inlinesvg smil svgclippaths"><head>
    ...
    <script src="https://hm.baidu.com/hm.js?adffbbfa4c220a656094574941c145dc"></script><script type="text/javascript" src="https://www.zhutibaba.com/demo/zimeiti2/wp-includes/js/jquery/jquery.js?ver=1.12.4-wp"></script>
    ...
    <div id="content" class="site-content container   clear">
       ...
       @include('index.layout.left')
                ...
                <main id="main" class="site-main">
                    ...
                    <article id="post-87" class="post-87 post type-post status-publish format-standard has-post-thumbnail hentry category-sports category-sub-1 category-sub-3 category-sub-2 category-house category-edu category-news category-travel category-auto category-tech category-food category-finance tag-25 tag-26 tag-27">
                        ...
                        <div class="single-breadcrumbs">
                            <span>您的位置</span> <i class="fa fa-angle-right"></i> <a href="{{route('index.index.index')}}">首页</a> <i class="fa fa-angle-right"></i> <a href="{{route('index.cate.index',['id'=>$articleInfo->cate_id])}} ">{{$articleInfo->cate->cate_name}}</a> 	</div>

                        <header class="entry-header">

                            <h1 class="entry-title">{{$articleInfo->title}}</h1>
                            <div class="entry-meta">




		                    <span class="entry-date">{{$articleInfo->created_at}}</span><!-- .entry-date -->



                            <span class="entry-views">阅读<span class="view-count">({{$articleInfo->click}})</span> </span>
                    ...
                    <div class="popular-content entry-related">
                        ...
                        <div class="popular-loop">
                            @foreach($artRes as $key=>$art)
                            <div class="hentry">
                                <h2 class="entry-title"><span class="post-num num-{{$key+1}}">{{$key+1}}</span><a href="{{route('index.article.index',['id'=>$art->id])}}">{{$art->title}}</a></h2>
                                <div class="entry-meta">
                                    <span class="entry-views">阅读<span class="view-count">({{$art->click}})</span> </span>
                                </div>
                            </div><!-- .hentry -->
                            @endforeach
                        </div><!-- .popular-loop -->
        ...
        <aside id="secondary" class="widget-area sidebar">
            <div id="search-2" class="widget widget_search"><form id="searchform" method="get" action="{{route('index.article.search')}}">
